 ...
 *      OnSkeetMeleeChip( survivor, hunter, damage, isOverkill )
 *      OnSkeetSniperChip( survivor, hunter, damage, isOverkill )
 *      OnHunterDeadstop( survivor, hunter )
 *      OnBoomerPop( survivor, boomer, shoveCount, Float:timeAlive )
 *      OnChargerLevel( survivor, charger )
 *      OnChargerLevelChip( survivor, charger, damage )
...
char
	g_sDebugFile[256];	  // debug file name

bool
	g_bLateLoad = false;	// whether we're loading late (after map has started)

GlobalForward
	g_hForwardSkeet,
	g_hForwardSkeetHurt,
	g_hForwardSkeetMelee,
	g_hForwardSkeetMeleeChip,
	g_hForwardSkeetSniper,
	g_hForwardSkeetSniperChip,
	g_hForwardSkeetGL,
	g_hForwardHunterDeadstop,
	g_hForwardSIShove,
	g_hForwardBoomerPop,
	g_hForwardLevel,
	g_hForwardLevelChip,
	...
	g_hForwardClear,
	g_hForwardVomitLanded,
	g_hForwardBHopStreak,
	g_hForwardAlarmTriggered;

StringMap
	g_hTrieWeapons,			 // weapon check
	g_hTrieEntityCreated,	 // getting classname of entity created
	g_hTrieAbility,			 // ability check
	g_hWitchTrie,			 // witch tracking (Crox)
	g_hRockTrie,			 // tank rock tracking
	g_hCarTrie;				 // car alarm tracking
...
// alarms
float				  g_fLastCarAlarm = 0.0;				   // time whenIsValidInfected last car alarm went off
CarAlarmTriggerReason g_LastCarAlarmReason[MAXPLAYERS + 1];	   // what this survivor did to set the last alarm off
	...
	AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	g_hForwardSkeetMeleeChip  = CreateGlobalForward("OnSkeetMeleeChip", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
	g_hForwardSkeetSniper	  = CreateGlobalForward("OnSkeetSniper", ET_Ignore, Param_Cell, Param_Cell);
	g_hForwardSkeetSniperChip = CreateGlobalForward("OnSkeetSniperChip", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
	...
	g_hForwardLevelChip		  = CreateGlobalForward("OnChargerLevelChip", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	g_hForwardCrown			  = CreateGlobalForward("OnWitchCrown", ET_Ignore, Param_Cell, Param_Cell);
	g_hForwardCrownChip		  = CreateGlobalForward("OnWitchCrownChip", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
...
public void OnPluginStart()
{
	...
	g_cvarRepChipSkeet		= CreateConVar("sm_skill_report_chipskeet", "0", "Enable hurt-skeet reporting. 0: disabled, 1: enabled, 2: if bOverKill is true, it will be output as skeet", FCVAR_NONE, true, 0.0, true, 2.0);
	g_cvarRepLevel			= CreateConVar("sm_skill_report_level", "1", "Enable level reporting.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarRepchipLevel		= CreateConVar("sm_skill_report_chiplevel", "1", "Enable hurt-level reporting.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarRepCrow			= CreateConVar("sm_skill_report_crow", "0", "Enable crow reporting.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarRepChipCrow		= CreateConVar("sm_skill_report_chipcrow", "0", "Enable draw-crow reporting.", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	g_cvarRockEaten			= CreateConVar("sm_skill_report_rockeaten", "1", "Enable rock-eaten reporting.", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	g_cvarRepVomitLanded	= CreateConVar("sm_skill_report_vomitlanded", "4", "Enable vomiting reporting. allows you to limit the report depending on the number of vomited people.", FCVAR_NONE, true, 0.0);
	...
	g_cvarHideFakeDamage	= CreateConVar("sm_skill_hidefakedamage", "1", "If set, any damage done that exceeds the health of a victim is hidden in reports.", FCVAR_NONE, true, 0.0, true, 1.0);
