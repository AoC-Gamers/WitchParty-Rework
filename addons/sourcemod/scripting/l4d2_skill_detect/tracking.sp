...
public Action Event_PlayerHurt(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		zClass		 = L4D2_GetPlayerZombieClass(victim);
				...
				if (isPouncing)
				{
						...
						*/
						...
						if (GetTrieValue(g_hTrieWeapons, weaponA, weaponTypeA) && (weaponTypeA == WPTYPE_SNIPER || weaponTypeA == WPTYPE_MAGNUM))
						{
							...
							else
							{
								// hurt skeet
								if (g_cvarAllowSniper.BoolValue)
									HandleChipSkeet(attacker, victim, damage, (g_iHunterOverkill[victim] + g_iHunterShotDmgTeam[victim] > g_iPounceInterrupt), false, true);
					...
					else if (damagetype & DMG_SLASH || damagetype & DMG_CLUB)
					{
						...
						else if (health == 0)
						{
							...
							if (g_cvarAllowMelee.BoolValue)
								HandleChipSkeet(attacker, victim, damage, true, true, false);
	...
	{
		zClass		 = L4D2_GetPlayerZombieClass(attacker);
		switch (zClass)
		{
			case L4D2ZombieClass_Hunter:
			{
...
public Action Event_PlayerDeath(Handle hEvent, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidInfected(victim))
	{
		L4D2ZombieClassType zClass = L4D2_GetPlayerZombieClass(victim);
		switch (zClass)
		{
			case L4D2ZombieClass_Hunter:
			{
				...
				if (g_iHunterShotDmgTeam[victim] > 0 && g_bHunterKilledPouncing[victim])
				{
					...
					if (g_iHunterShotDmgTeam[victim] > g_iHunterShotDmg[victim][attacker] && g_iHunterShotDmgTeam[victim] >= g_iPounceInterrupt)
						...
						HandleChipSkeet(attacker, victim, g_iHunterShotDmgTeam[victim], (g_iHunterOverkill[victim] + g_iHunterShotDmgTeam[victim] > g_iPounceInterrupt));
					else
						// not a skeet at all
						HandleChipSkeet(attacker, victim, g_iHunterShotDmg[victim][attacker]);
				}
				else 
				{
					// check whether it was a clear
					if (g_iSpecialVictim[victim] > 0)
						HandleClear(attacker, victim, g_iSpecialVictim[victim], L4D2ZombieClass_Hunter, (GetGameTime() - g_fPinTime[victim][0]), -1.0);
				}
			...
			case L4D2ZombieClass_Jockey:
			{
				// check whether it was a clear
				if (g_iSpecialVictim[victim] > 0)
					HandleClear(attacker, victim, g_iSpecialVictim[victim], L4D2ZombieClass_Jockey, (GetGameTime() - g_fPinTime[victim][0]), -1.0);
			}
			...
			case L4D2ZombieClass_Charger:
			{
				...
				// check whether it was a clear
				if (g_iSpecialVictim[victim] > 0)
					HandleClear(attacker, victim, g_iSpecialVictim[victim], L4D2ZombieClass_Charger, (g_fPinTime[victim][1] > 0.0) ? (GetGameTime() - g_fPinTime[victim][1]) : -1.0, (GetGameTime() - g_fPinTime[victim][0]));
			}
		}
	}
...
public Action Event_PlayerShoved(Handle event, const char[] name, bool dontBroadcast)
{
	...
	// track on boomers
	if (zClass == L4D2ZombieClass_Boomer)
		...
		switch (zClass)
		{
			case L4D2ZombieClass_Hunter:
			{
				if (GetEntPropEnt(victim, Prop_Send, "m_pounceVictim") > 0)
					HandleClear(attacker, victim, GetEntPropEnt(victim, Prop_Send, "m_pounceVictim"), L4D2ZombieClass_Hunter, (GetGameTime() - g_fPinTime[victim][0]), -1.0, true);
			}
			case L4D2ZombieClass_Jockey:
			{
				if (GetEntPropEnt(victim, Prop_Send, "m_jockeyVictim") > 0)
					HandleClear(attacker, victim, GetEntPropEnt(victim, Prop_Send, "m_jockeyVictim"), L4D2ZombieClass_Jockey, (GetGameTime() - g_fPinTime[victim][0]), -1.0, true);
			}
		}
	}
...
public Action Event_PlayerJumped(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		L4D2ZombieClassType zClass = L4D2_GetPlayerZombieClass(client);
		if (zClass != L4D2ZombieClass_Jockey)
			return Plugin_Continue;
...
stock void CheckWitchCrown(int witch, int attacker, bool bOneShot = false)
{
	...
	else if (witch_dmg_array[MAXPLAYERS + WTCH_CROWNSHOT] >= g_cvarDrawCrownThresh.IntValue)
	{
		...
		HandleChipCrown(attacker, witch_dmg_array[MAXPLAYERS + WTCH_CROWNSHOT], chipDamage);
	}
...
public Action OnTakeDamage_Car(int victim, int& attacker, int& inflictor, float& damage, int& damagetype)
{
	...
	*/
	...
	{
		if (IsValidInfected(inflictor) && L4D2_GetPlayerZombieClass(inflictor) == L4D2ZombieClass_Boomer)
		{
			g_LastCarAlarmReason[attacker] = CarAlarmTrigger_Boomer;
			g_iLastCarAlarmBoomer			= inflictor;
		}
		else 
			g_LastCarAlarmReason[attacker] = CarAlarmTrigger_Explosion;
	}
	else if (damage == 0.0 && (damagetype & DMG_CLUB || damagetype & DMG_SLASH) && !(damagetype & DMG_SLOWBURN))
		g_LastCarAlarmReason[attacker] = CarAlarmTrigger_Touched;
	else
		g_LastCarAlarmReason[attacker] = CarAlarmTrigger_Hit;
...
public void OnTouch_Car(int entity, int client)
{
	...
	g_LastCarAlarmReason[client] = CarAlarmTrigger_Touched;
...
public Action Timer_CheckAlarm(Handle timer, any entity)
{
	...
	if ((GetGameTime() - g_fLastCarAlarm) < CARALARM_MIN_TIME)
	{
		...
		{
			if (g_LastCarAlarmReason[survivor] == CarAlarmTrigger_Boomer)
		...
		HandleCarAlarmTriggered(survivor, infected, (IsValidClientInGame(survivor)) ? g_LastCarAlarmReason[survivor] : CarAlarmTrigger_Unknown);
	}
	return Plugin_Continue;
}
